...
# modified from Stanford website, more concise than below
	...
	for ((i=1;i<=${fastfiles}/2;i++)); do
		...
		for ((j=1;j<=2;j++)); do
			...
			mv -n ${tempfast} sub-${subj}_task-FAST_acq-${tp}TP_bold.${tempfastext} #change the filenames
			echo "$tempfast changed to sub-${subj}_task-FAST_acq-${tp}TP_bold.${tempfastext}"
		done
	done
