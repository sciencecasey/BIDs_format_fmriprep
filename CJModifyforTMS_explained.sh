#!/bin/bash
#original author: Angel Wong
#modified for VM_TMS by Casey Jayne 7/14/2019

set -e  #to automatically exit out if the script command exits with a "non-zero" status

####Defining pathways
toplvl=/Users/casey/Desktop/working_folder/bids_structure_practice  #set this directory to the top level folder you want to move things into
dcmdir=/Users/casey/Desktop/working_folder/bids_structure_practice/dicom/BDD		# For BDD subjects  #set this to dicom group 1 folder
#dcmdir=/Users/casey/Desktop/working_folder/bids_structure_practice/dicom/CON		# For CON subjects #set this to dicom group 2 folder
softwaredir=/usr/local/Cellar
#my dcm2niidir=/usr/local/Cellar ???

###Create dataset_description.json
jo -p name="BDD TMS Visual Modulation and Rest Datasets" "BIDSVersion"="1.0.2" >> ${niidir}/dataset_description.json
...
for subj in 10001; do  #change this to the folder you want it to be
	...
	#edited to change the .nii to .nii.gz
	...
	#NOT ALL PARTICIPANTS HAD A DWI FOR THIS SCAN
	cd ${dcmdir}/TMS_VM_${subj}/dicom
	#if test -n "$(find . -maxdepth 1 -name '*DWI*' -print -quit)"
	#then
		for direcs in *DWI*; do
			dcm2niix -b y -o ${niidir}/sub-${subj} -f ${subj}_%p_%s ${dcmdir}/TMS_VM_${subj}/dicom/${direcs} #convert the filenames
			#Change directory into the subject folder
			cd ${niidir}/sub-${subj}
			difffiles=$(ls -1 *DWI* | wc -l) #capture the number of files to change
			for ((i=1;i<=${difffiles};i++)); do
				Diff=$(ls *DWI*) #This is to refresh the diff variable, same as the cases above.
				tempdiff=$(ls -1 $Diff | sed '1q;d') #grab the first file
				tempdiffext="${tempdiff##*.}" #save its extension
				tempdifffile="${tempdiff%.*}" #save it's filename
				mv --backup=numbered ${tempdifffile}.${tempdiffext} sub-${subj}_dwi.${tempdiffext} #change the filenames
	...
	#else #if subject didn't complete a DWI
  #  echo no DWI for sub $subj found
	#fi
	...
	for direcs in VIS_MOD_1_* VIS_MOD_2_* VIS_MOD_3_* FAST* REST*; do
		...
		dcm2niix -o ${niidir}/sub-${subj} -f ${subj}_%p_%s ${dcmdir}/TMS_VM_${subj}/dicom/${direcs}
	done
	...
	for ((i=1;i<=${vismodfiles};i++)); do
		...
		vismod=$(ls *VIS_MOD*) #Capture new file to change
		tempvismod=$(ls -1 $vismod | sed '1q;d')
		...
		mv -n ${tempvismod} sub-${subj}_task-${taskname}_bold.${tempvismodext}
		#always backup so that you don't rewrite the filenames
		#this makes a ~1~ if there are more than one
		echo "${tempvismod} changed to sub-${subj}_task-${taskname}_bold.${tempvismodext}"
	done

	fastfiles=$(ls -1 *FAST* | wc -l)
	for ((i=1;i<=${fastfiles};i++)); do
		fast=$(ls *FAST*)
		fast=$(ls *FAST*)
		...
		mv -n ${tempfast} sub-${subj}_task-${taskname}_bold.${tempfastext} #change the filenames
		#Example filename: 5015_DWI_dir99_AP_7
		#BIDS filename: sub-5015_dwi
		echo "$tempfast changed to sub-${subj}_task-${taskname}_bold.${tempfastext}"
		###Organize files into folders
	done
...
#tp is a numerical vaiable with the number of volumes within that functional data (570 for 7 min, 406 for 5 min)
	...
	if ls *~* 1> /dev/null 2>&1; then #look for ~ to indicate a file that has the backup rest because there is more than one rest of the same length
		...
		mv sub*TP* func #what is the TP here?
		cd func
		((newnum=$dupnum+1))
		for file in *TP_*; do
	...
	for niifiles in *.nii; do
		mv -- "$niifiles" "${niifiles%.nii}.nii.gz"
	done
